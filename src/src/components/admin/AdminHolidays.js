...
const TYPES = { friday: 'Friday', eid: 'Eid', puja: 'Puja', national: 'National Holiday', semester_break: 'Semester Break', other: 'Other' };
    ...
    <div>
      ...
      <div className="page" style={{ paddingTop: 8 }}>
        <div className="info-banner mb-3">
          ...
          <span className="info-text">An attendance session cannot be started on a Holiday. It is excluded from Percentage calculations.</span>
        </div>
        {loading ? <div className="loading"><div className="spinner" /></div> : (
          <div className="card">
            {holidays.length === 0 ? <div className="empty"><Icon name="calendar" size={32} /><p>No holidays yet</p></div> : holidays.map(h => (
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)}>
            <h3 className="modal-title">New Holiday</h3>
            <form onSubmit={handleCreate}>
              <div className="form-group"><label className="form-label">Title *</label><input className="form-input" required placeholder="e.g. Eid ul Fitr" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} /></div>
              ...
              <div className="form-group"><label className="form-label">Description</label><input className="form-input" placeholder="Description (optional)" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
